...
export function buildStreamPath(data) {
  const prefixA = decodeString(2768, '50Br')
  const prefixB = decodeString(1932, 'oInb')
  return `${prefixA}/${prefixB}/${data}`.replace(/\/+/g, '/').replace(/^\//, '')
}
...
export function parseStreamRequest(searchParams) {
  ...
  const contentPath = buildContentPath({ type, id, season, episode })
  return { id, type, season, episode, contentPath }
}
