...
const CHUNK_PATH = path.join(__dirname, '../assets/chunk-365.js')
...
export function createVmRuntime(env = {}) {
  ...
  const g = {
    ...
    location: env.location || {
      hostname: 'www.vidfast.net',
      host: 'www.vidfast.net',
      origin,
  ...
  const body = buildVmScript() + '; return globalThis.__vidfast;'
  const fn = new Function(
    'globalThis', 'window', 'self', 'document', 'navigator', 'localStorage', 'screen', 'crypto', 'fetch', 'atob', 'btoa', 'Buffer', 'TextEncoder', 'TextDecoder', 'location', 'performance',
    body,
  )
  return fn(g, g, g, g.document, g.navigator, g.localStorage, g.screen, g.crypto, g.fetch, g.atob, g.btoa, g.Buffer, g.TextEncoder, g.TextDecoder, g.location, g.performance)
}
